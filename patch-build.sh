...
perl -i -pe "$@" 'next if /\( startgroup "Validating outputs" / .. /\( endgroup "Validating outputs" /' $BUILD_STEPS_LINUX
echo "✅ Deleting 'Validating outputs' block in $BUILD_STEPS_LINUX"
perl -i -pe "$@" 'next if /\( startgroup "Validating outputs" / .. /\( endgroup "Validating outputs" /' $BUILD_STEPS_OSX
